@php
    ...
    $pilihan = $cekJawaban ? strtoupper(trim((string) $cekJawaban->pilihan)) : '';
@endphp
...
<table class="
    ...
    <tbody>
        ...
        'E' => $detailsoal->pile,
            ...
            <tr id="wrap_pil_{{ strtolower($kode) }}" class="{{ $pilihan === $kode ? 'benar' : '' }}">
                <td style="width:10px;">
                    <input type="radio" name="pilih{{ $detailsoal->id }}" value="{{ $kode }}"
                        {{ $pilihan === $kode ? 'checked' : '' }}>
                </td>
