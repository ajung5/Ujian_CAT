<table
    ...
    <tbody>
        @forelse ($soals as $soal)
            <tr>
                <td>
                    {{ method_exists($soals, 'firstItem') ? $soals->firstItem() + $loop->index : $loop->iteration }}
                </td>
                ...
                <td>
                    {{ $soal->created_at ? $soal->created_at->format('d M Y') : '-' }}
                </td>
...
@if (method_exists($soals, 'links'))
    {{ $soals->links('vendor.pagination.bootstrap-3') }}
@endif
